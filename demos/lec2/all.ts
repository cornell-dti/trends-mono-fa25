...
const getArea = (shape: Shape): number => {
  if ("radius" in shape) {
    // Narrowing to Circle type. Since everything stripped after compilation!
    return Math.PI * shape.radius ** 2;
  } else {
    // Narrowing to Rectangle type
...
const introduce = (userName: string, github: string = "no-github"): string =>
  github
    ? `Hi, I'm ${userName}. My GitHub is @${github}`
    : `Hi, I'm ${userName} and I have no GitHub 😢 .`;
